...
function shouldProxyAppServer(args) {
  ...
  if (hasAppServerSubcommand(appServerArgs)) return false;
  return true;
}
...
function hasAppServerSubcommand(args) {
  ...
  for (const argument of args) {
    ...
    if (argument.startsWith("-")) {
      ...
      if (APP_SERVER_VALUE_OPTIONS.has(option) && !argument.includes("=")) {
        expectsValue = true;
      }
      continue;
    }
...
async function runSelfTest() {
  ...
  if (shouldProxyAppServer(["app-server", "future-command"])) {
    throw new Error("Desktop proxy self-test failed: unknown subcommands must be delegated");
  }
  if (!shouldProxyAppServer(["-c", "foo=true", "app-server", "--analytics-default-enabled"])) {
    throw new Error("Desktop proxy self-test failed: app-server options must remain proxied");
  }
